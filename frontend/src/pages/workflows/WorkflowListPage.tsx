import { Box, Typography, Button, Card, CardContent, Grid, Chip, IconButton, TextField, InputAdornment, Tab, Tabs, CircularProgress, Alert, Pagination } from '@mui/material';
import { Add as AddIcon, Search as SearchIcon, FilterList, ViewList, GridView, MoreVert } from '@mui/icons-material';
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useWorkflowsPaginated } from '../../hooks/useWorkflows';

const statusColors: Record<string, string> = {
  RUNNING: '#4A90D9', COMPLETED: '#1BA672', FAILED: '#E23744', BLOCKED: '#F7A83E', CANCELLED: '#9C9C9C',
};

const statusFilters = ['', 'RUNNING', 'COMPLETED', 'FAILED'];
...
export function WorkflowListPage() {
  ...
  const { data, isLoading, error } = useWorkflowsPaginated({
    status: statusFilters[tab] || undefined,
  ...
  return (
    <Box>
      ...
      <Card sx={{ mb: 3, borderRadius: 2 }}>
        <CardContent sx={{ pb: '0 !important' }}>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', mb: 1 }}>
            <TextField size="small" placeholder="Search workflows..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }}
      ...
      {isLoading ? (
        ...
        <Alert severity="error" sx={{ borderRadius: 2 }}>Failed to load workflows</Alert>
      ) : workflows.length === 0 ? (
        ...
        <>
          <Card sx={{ borderRadius: 2 }}>
            {workflows.map((wf, i) => (
              <Box key={wf.id} sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderBottom: i < workflows.length - 1 ? 1 : 0, borderColor: 'divider', cursor: 'pointer', '&:hover': { bgcolor: '#FFF5F5' } }} onClick={() => navigate(`/workflows/${wf.id}`)}>
