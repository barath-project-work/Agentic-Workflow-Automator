...
import { Box, Typography, Card, CardContent, Chip, IconButton, Tabs, Tab, Checkbox, Button, Grid } from '@mui/material';
...
export function TaskListPage() {
  ...
  const filtered = tab === 0 ? demoTasks : demoTasks.filter(t => t.status === ['PENDING', 'IN_PROGRESS', 'COMPLETED', 'FAILED'][tab - 1]);
      ...
      <Card sx={{ mb: 3, borderRadius: 2 }}>
        <CardContent sx={{ pb: '0 !important' }}>
          <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ '& .MuiTab-root': { textTransform: 'none', fontWeight: 500, fontSize: '0.875rem', minHeight: 48 }, '& .Mui-selected': { color: '#E23744 !important' }, '& .MuiTabs-indicator': { bgcolor: '#E23744' } }}>
            <Tab label={`All (${demoTasks.length})`} />
            <Tab label={`Pending (${demoTasks.filter(t => t.status === 'PENDING').length})`} />
            <Tab label={`In Progress (${demoTasks.filter(t => t.status === 'IN_PROGRESS').length})`} />
            <Tab label={`Completed (${demoTasks.filter(t => t.status === 'COMPLETED').length})`} />
          </Tabs>
        </CardContent>
      </Card>
              ...
              <CardContent sx={{ display: 'flex', alignItems: 'center', gap: 2, py: 2, '&:last-child': { pb: 2 } }}>
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {task.type.replace('_', ' ')} · Due {new Date(task.dueDate).toLocaleDateString()} · {task.relatedEntityName && `${task.relatedEntityName}`}
                  </Typography>
                </Box>
