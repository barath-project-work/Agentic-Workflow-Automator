...
import { Box, Typography, Card, CardContent, Chip, Button, ToggleButton, ToggleButtonGroup } from '@mui/material';
import { useNavigate } from 'react-router-dom';
import { Add, ViewKanban, TableChart } from '@mui/icons-material';
import { demoOpportunities, pipelineData } from '../../services/mockData';

const stageColors: Record<string, string> = { PROSPECT: '#9C9C9C', QUALIFIED: '#4A90D9', PROPOSAL: '#F7A83E', NEGOTIATION: '#E67E22', WON: '#1BA672', LOST: '#E23744' };
...
export function OpportunityListPage() {
      ...
      <Box>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            <Button variant="contained" startIcon={<Add />} sx={{ borderRadius: 2, py: 1, px: 3 }}>New Deal</Button>
          </Box>
        </Box>
            ...
            <Box sx={{ display: 'flex', gap: 0.5 }}>
              {pipelineData.map((p, i) => (
                <Box key={p.stage} sx={{ flex: 1, textAlign: 'center' }}>
                  <Box sx={{ height: p.value / 50000, bgcolor: stages[i] === 'WON' ? '#1BA672' : '#E23744', opacity: 0.7 + (i * 0.05), borderRadius: '4px 4px 0 0', minHeight: 20, transition: 'height 0.3s' }} />
        ...
        <Box sx={{ display: 'flex', gap: 1.5, overflow: 'auto', pb: 1 }}>
          {stages.map(stage => {
            const stageOpps = demoOpportunities.filter(o => o.stage === stage);
            return (
                    ...
                    <Card key={opp.id} sx={{ borderRadius: 2, cursor: 'pointer', '&:hover': { boxShadow: '0 2px 12px rgba(0,0,0,0.1)' } }} onClick={() => navigate(`/opportunities/${opp.id}`)}>
                      <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
                          <Typography variant="body2" fontWeight={700} color="primary">₹{opp.value.toLocaleString()}</Typography>
  ...
  return (
    <Box>
      <Typography variant="h5" fontWeight={700} mb={3}>Opportunities</Typography>
      <Card sx={{ borderRadius: 2 }}>
        {demoOpportunities.map((opp, i) => (
          <Box key={opp.id} sx={{ display: 'flex', alignItems: 'center', gap: 2, p: 2, borderBottom: i < demoOpportunities.length - 1 ? 1 : 0, borderColor: 'divider', cursor: 'pointer', '&:hover': { bgcolor: '#FFF5F5' } }}>
            <Box sx={{ flex: 1 }}>
            ...
            <Typography variant="body2" fontWeight={700}>₹{opp.value.toLocaleString()}</Typography>
